...
DWORD getProcessMainThreadid(DWORD dwProcID)
{
...
int main( int argc, char**argv )
{
	...
	while (!(GetAsyncKeyState(VK_F5) & 0x8000))
	{
		if(flag)
		{
			TlsHookController = Mp_MapDllAndCreateHookEntry(DllPath, _TlsGetValue, Target, !Flags["noloadlib"], [&](SIZE_T Size)
			{
				//return VirtualAlloc( 0, Size, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE );
				//PVOID Memory = AllocateKernelMemory(CpCtx, KrCtx, Size);
				//ExposeKernelMemoryToProcess(Controller, Memory, Size, Controller.CurrentEProcess);
				ZeroMemory(UsedRegions[0].first, Size);
				//UsedRegions.push_back({ Memory, Size });
				return UsedRegions[0].first;
			});
		}
	printf( "\n" );
	...
	auto Tid = getProcessMainThreadid(Pid);
	...
	{
		0x65, 0x48, 0x8B, 0x04, 0x25, 0x48, 0x00, 0x00, 0x00,        // mov rax, gs:[0x48]
		0x90, 0x90, 0x90,                                            // nop/////////mov eax,[rax+0x40] ; pid
	...
	*(uint32_t*)(&PidBasedHook[0xD]) = Tid; // Pid
